...
onMounted(() => {
  ...
  document.title = `"${searchQuery.value}" 검색 결과`;
});
</script>
...
<template>
  <main class="page">
    <div class="header-section">
      <h1>검색 결과</h1>
      <p class="sub-title">"{{ searchQuery }}" 에 대한 결과입니다.</p>
    </div>
    ...
    <div v-else-if="!searchQuery" class="status-message empty">
      검색어를 입력해 주세요.
    </div>
    <div v-else-if="filteredMovies.length === 0" class="status-message empty">
      "<strong>{{ searchQuery }}</strong>" 에 해당하는 영화가 없습니다.
    </div>
    <div v-else>
      <p class="result-count">총 <strong>{{ filteredMovies.length }}</strong>편의 영화를 찾았습니다.</p>
        ...
        <div v-for="movie in filteredMovies" :key="movie.id" class="movie-card">
          ...
          <div class="card-content">
            ...
            <button
              ...
              class="fav-btn"
            >{{ movie.isFavorite ? '♥ 찜 해제' : '♡ 찜하기' }}</button>
          </div>
        </div>
      </div>
    </div>
...
.header-section { margin-bottom: 24px; text-align: center; color: #2c3e50;}
.sub-title { font-size: 14px; color: #7f8c8d; margin-top: 5px;}
.result-count { font-size: 14px; color: #555; margin-bottom: 24px;}
